...
for i in xrange(0, feature_num + 1):
    model.append(random.uniform(-1, 1))
...
for i in xrange(0, max_iter):
    ...
    for sample in samples:
        ...
        label_predict = util.sigmoid(label_predict)
        # print "DEBUG: sample %s label %f predict %f predict - real %f" % (attr[0], label_real, label_predict, label_predict - label_real)
        cost_function_value = cost_function_value + label_real * math.log(label_predict) + (1 - label_real) * math.log(1 - label_predict)
    ...
    cost_function_value = -(cost_function_value / sample_num)
